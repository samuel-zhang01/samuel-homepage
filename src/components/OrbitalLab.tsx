...
export default function OrbitalLab({ locale, active = true }: { locale: Locale; active?: boolean }) {
  ...
  return <div className={`system7-project ${styles.app}`}>
    ...
    <section className={styles.notes}><h3>{c.sources}</h3><h4>{c.model}</h4><p>{c.modelBody}</p><p>{c.phaseBody}</p><p>{c.projectionBody}</p><p>{c.pointsNote}</p><p>{c.surfaceNote}</p><p>{c.scaleBody}</p><p>{c.configBody}</p><div><a href={ORBITAL_SOURCES.configurations} target="_blank" rel="noreferrer">{c.sourceNist} ↗</a><a href={ORBITAL_SOURCES.hydrogen} target="_blank" rel="noreferrer">{c.sourceMath} ↗</a><a href={ORBITAL_SOURCES.elementIndex} target="_blank" rel="noreferrer">NIST · 93–104 ↗</a></div></section>
  </div>;
}
