...
try {
  ...
  await check("CV draft and selected tab survive desktop focus and the 760px boundary", async () => {
    ...
    await p.getByRole("button", { name: "Open interactive demo", exact: false }).click();
    ...
    for (const size of [{ width: 390, height: 844 }, { width: 1440, height: 1000 }]) {
      ...
      await p.setViewportSize(size);
      assert.equal(await p.locator("#interactive-lab-cv-keyword-automator").count(), 1);
    ...
    for (const size of [{ width: 320, height: 568 }, { width: 390, height: 844 }]) {
      await p.setViewportSize(size);
      await p.getByRole("button", { name: "Return to demo", exact: false }).click();
      await p.waitForTimeout(500);
      const detail = p.locator('[id$="-detail"]');
    ...
    await p.getByRole("button", { name: "Close demo and return to overview", exact: false }).click();
  ...
  await check("Mandarin demo launch fits a 320px pane in both locales", async () => {
    for (const locale of ["zh-cn", "zh-tw"]) {
      ...
      await p.locator('[data-embedded="true"] button[aria-expanded]').first().click();
      const experiment = p.locator("#interactive-lab-cv-keyword-automator");
      await experiment.locator('[aria-describedby="cv-studio-cv-help"]').waitFor();
      await p.locator('[data-embedded="true"] button[aria-expanded]').first().click();
      await p.waitForTimeout(500);
      const pane = await p.locator('[id$="-detail"]').boundingBox();
  ...
  await check("MRI saved image and limitations agree in English and both Mandarin editions", async () => {
    for (const [locale, images, limits, phrase] of [
      ...
      const p = await page(`/${locale}/projects?project=trustworthy-mri-reconstruction`);
